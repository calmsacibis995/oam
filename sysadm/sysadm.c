...
main(int argc, char **argv)
{
	...
	if(argc > 2) {
		if((*argv[1] == '-') && (strcmp(nnamep, SYSADM) == 0)) {
	 ...
	 * Check for the "sysadm" user if present on the system.
	 */
...
#else
	/*
	 * There is no need for "su -" in Linux, as we can just
	 * run "sysadm" with sudo instead. This also eliminates the
	 * requirement for the creation of the "sysadm" account.
	 */
	if(geteuid() != userp->pw_uid) {
		printf("Insufficient privileges.\n");
		fprintf(stderr, "Run the command as the superuser and try again.\n");
		exit(1);
	}
#endif
...
get_term()
{
	(void) printf("I don't know your terminal. Could you please tell me?\n");
	(void) printf("Enter your terminal type: ");
	return(fgets(inbuf, sizeof(inbuf), stdin));
}
