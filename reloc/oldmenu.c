...
char do_inline[LNSZ];		/* input buffer */
char filename[LNSZ];		/* file or directory name */

char *get_desc();

struct old_item *
input_oldmenu(char *name)
{
...
char *filename;		/* name of file to get description from */
{
	...
	for(;;) {
		if(fgets(do_inline, sizeof(do_inline), fptr) == NULL) break;
		if(strncmp(do_inline, MENUHDR, strlen(MENUHDR)) == 0) {
			found = 1;
			pos = strrchr(do_inline, '\n');
			if(pos != NULL) *pos = NULL;
			break;
		}
	}
	if(found) return(do_inline + strlen(MENUHDR) + 1);
	else return(NULL);
